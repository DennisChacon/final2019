...
Data came from a study in 2017, avast, used for a final project which was modified for the assignment.   
...
$H_0$: The vaccine is ineffective. Avacavirinoal has no effect on whether one contracts AIDS or not.           
$H_A$: The vaccine is effective. Avacavirinoal has an effect on whether one contracts AIDS or not. 
...
A barchat to show the results on the general population.  


### Graphical Descriptive Results

The following bar chart demonstrates that the placebo had little to no effect on the infection rate. However there is a drop in the infection rate in those who were administered the vaccine.
...
Infering from the bar chart, numeric table and the chi-squared test the results demonstrate that the vaccine has an effect and is reasonably effective on the general population. 
...
The p-value is  0 , so we reject the null hypothesis which is smaller than .05. Because the response and explanatory values are categorical, the bar plot and chi-squared test | test for independence.  
...
A bar chart was used to show the results of the effect sex has on the vaccine

### Graphical Descriptive Results

The following bar chart shows that the vaccine appears to be similarly effective on males and females.
...
$H_0$: The vaccine is ineffective. Avacavirinoal has no effect on whether one will contract AIDS or not.           
$H_A$: The vaccine is effective. Avacavirinoal has an effect on whether one will contract AIDS or not.     


The hypothesis was that gender had no effect on the effectiveness of the vaccine.  

The p-value was<  0.3048 , so we fail to reject the null hypothesis which is greater than .05. Because the response and explanatory values are categorical, the bar plot and chi-squared test | test for independence.  
...
Does use of drug use by injection make a difference in vaccine effectiveness    
...
The following bar chart shown demonstrates that the vaccine appears to be slightly more effective on non-users than the users.
...
A bar chart WAS USED to show the effectiveness of the vaccine on the LBGT population.  
...
The p-value was  0 , so the null hypothesis was rejected which was smaller than .05. Because the response and explanatory values are categorical, the bar plot and chi-squared test | test for independence.  



GENDER
The results demonstrates that the vaccine is reasonably effective on the general population. 
...
IV DRUG
The results demonstrate that the vaccine is only slightly effected by gender at all.    
The null hypothesis was that IV Drug use had no effect on the effectiveness of the vaccine.
